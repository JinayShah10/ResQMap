      ...
      <div className="relative z-10 w-full max-w-md bg-slate-900/90 backdrop-blur-lg border border-slate-700/40 rounded-2xl p-6 md:p-8 shadow-[0_25px_60px_rgba(0,0,0,0.85)] flex flex-col justify-center animate-fade-in-header auth-card-floating">
        ...
        <div className="text-center mb-8">
          <div className="inline-flex w-12 h-12 rounded-xl bg-emerald-500 items-center justify-center shadow-lg shadow-emerald-500/20 mb-3 active:scale-95 transition-transform duration-200">
            <svg className="w-5 h-5 text-slate-950 font-bold animate-pulse" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth="2.5">
          ...
          <p className="text-[10px] text-slate-400 uppercase tracking-widest mt-1">Geospatial Emergency Portal</p>
        </div>

        <div className="mb-6 p-3 bg-slate-800/40 border border-slate-700/50 rounded-lg text-xs text-slate-300 leading-relaxed text-center">
          <span className="text-emerald-400 font-semibold uppercase tracking-wider block mb-1">Integration Status</span>
          Secure database connection active. Real-time signup is fully operational.
        </div>

        {error && (
          <div className="mb-6 p-3 bg-rose-500/10 border border-rose-500/20 text-rose-400 rounded-lg text-sm font-medium">
            {error}
          </div>
        )}

        {infoMessage && (
          <div className="mb-6 p-3 bg-emerald-500/10 border border-emerald-500/20 text-emerald-400 rounded-lg text-xs leading-normal">
            {infoMessage}
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          <div className="space-y-1">
            ...
            <input
              ...
              className="w-full bg-slate-950/60 border border-slate-800 rounded-xl px-4 py-3 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-all"
              disabled={isLoading}
            />
          </div>
          ...
          <div className="space-y-1">
            ...
            <div className="relative">
              <input
                ...
                placeholder="••••••••"
                className="w-full bg-slate-950/60 border border-slate-800 rounded-xl pl-4 pr-12 py-3 text-sm text-slate-100 placeholder-slate-500 focus:outline-none focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/50 transition-all"
                disabled={isLoading}
              />
              <button
                type="button"
