...
3. View nearby matching facilities on an interactive map supporting both a conventional 2D map view and a perspective/3D map view.
4. Select a facility to view its details.
5. Get directions and route visualization to the chosen facility.

The core objective remains helping users discover nearby emergency facilities and get directions to them. The map visualization modes (2D and 3D) are options for displaying data and do not change or interrupt the underlying emergency facility discovery workflow.
...
4. **Map Display:** The user views their location and nearby facility markers on an interactive map. The user can switch between a conventional top-down 2D view and a perspective/3D view.
...
* **Mapping Library:** MapLibre GL JS + React-compatible MapLibre integration
